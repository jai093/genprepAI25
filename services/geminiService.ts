...
// Use GEMINI_API_KEY to align with standard practices and documentation.
const API_KEY = process.env.GEMINI_API_KEY;

if (!API_KEY) {
  // Updated console message to reflect the correct environment variable name.
...
export const generateInterviewQuestions = async (resume: ResumeData, targetRole: string, linkedinUrl?: string): Promise<string[]> => {
    ...
    let prompt = `You are an expert interviewer hiring for a "${targetRole}" position. You must generate 5 challenging interview questions tailored to the candidate's specific background.
...
2.  Tailor at least 2-3 questions directly to the candidate's experience (e.g., "Tell me about a project you worked on at ${resume.experience[0]?.company || 'your previous role'}" or "How did your experience with ${resume.skills[0] || 'a key skill'} help you succeed as a ${resume.experience[0]?.jobTitle || 'in your previous role'}?").
3.  The questions should be a mix of behavioral and role-specific challenges.
